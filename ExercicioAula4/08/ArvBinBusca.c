...
int contar_NoDeUmFilho_No(Nodo* raiz){
    while (raiz!=NULL){
        int contador = 0;
        if(verificar_filho (raiz)==1)
            contador++;
        contador += contar_NoDeUmFilho_No (raiz->esq);
        contador += contar_NoDeUmFilho_No (raiz->dir);
        return contador;
    }
}
void contar_NoDeUmFilho_ABB(ArvBin* arv){
    printf ("O numero de No com um Filho é: %d\n",contar_NoDeUmFilho_No (arv->raiz));
}